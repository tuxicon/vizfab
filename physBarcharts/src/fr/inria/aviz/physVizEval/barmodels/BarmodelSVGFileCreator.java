...
public class BarmodelSVGFileCreator extends javax.swing.JFrame implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    protected double barWidth = 3;
    protected double barSpacing = 3;
    protected double sliceWidth = 10 * barWidth + 11 * barSpacing; // total width for a 10 x 10 bar chart; will be changed if a bigger or smaller data set is loaded
    protected double baseCoverThickness = 3; // the thickness of the grill that is slit over the bars and only used for optical reasons 
//    protected double maxBarHeight = 60;
    protected double baseHeight = 22;// the height of the base that shows the lables
    protected Slice[] slices; // array holding 1 slice for each row of the data set
    protected Spacer[] spacers; // array of spacers that go between all slices, so that they don't touch
    protected BottomPlate bottom; // bottom with holes to facilitate assembling the bar chart
    protected Scale leftScale, rightScale; // the scale plates that go in the back; those have to be engraved with reversed writing as the acrylic looks better that way
    protected BaseLabel[] base; // the four sides of the base showing the labels
    protected int numberOfTicks = 9; // how many ticks the scales should contain 
    protected String[] ticks; // the labels for the ticks
    protected double maxTick; // the value of the highest tick
    protected double scaleHeight = 50; //72;// the height of the scales, i.e., the max height of a bar
    protected double maxTickHeight = (scaleHeight - 3);// the highest tick is placed 3mm from the upper edge of the scale
	protected double scaleFactor = maxTickHeight/maxTick; // a scale factor to allow easy re-scaling of the bar heights
	protected double thickness = 3;// thickness of the material used for base, scales, and bottom plate
	...
	protected static boolean displayOnScreen = true;
    ...
    public void paint(Graphics g) {
        ...
        // set a background color and allow pan & zoom if displayed on the screen
        if (displayOnScreen) {
...
//        Rectangle2D rectangle = new Rectangle2D.Float(100, 100, 240, 160);
        ...
        // draw all slices with thin lines that will be cut by the laser
...
//        	graphics2.rotate(0);
        ...
        GUIUtils.drawText(graphics2, data.getDataInfo().getTitle(), base[3].getBounds().getCenterX() - 5, base[3].getBounds().getMaxY() - thickness - barSpacing, Double.MAX_VALUE, Double.MAX_VALUE, GUIUtils.HALIGN.Right, GUIUtils.VALIGN.Top, Math.PI/2, null, null, 1, false);
...
//            graphics2.setTransform(at);
    ...
    private void initComponents(String _filename) {
		...
		// Let the user choose log files to display and process
        filename = _filename;
        // create the matrix structure to hold the data
		...
		// use maxY to store where we are currently drawing stuff
		double maxY = 0;
		// lets create all the slices first
		for (int i = 0 ; i < slices.length; i++) {
		...
		baseCover = new BaseCover(bottom.getBounds().getMinX(), bottom.getBounds().getMaxY() + barWidth, sliceWidth, sliceWidth, thickness, 2.75, 2.75, 2.75, 2.75, data.rows, data.cols, barWidth + barSpacing - baseCoverThickness, baseCoverThickness);
